...
3. P1 병사: **WASD 이동 · 마우스 조준/좌클릭 사격 · Q 수류탄 · F/E/Space 보급 요청**
4. P2 보급병: **방향키 이동 · Enter 준비/배송 · /를 누른 채 배송하면 오버차지 · . 품목 변경 · Shift 배송 대상 변경**
...
- 로컬 인간 **2명** + CPU **4명** (병사와 보급병의 비대칭 협동)
- 아트 없이도 핵심 판정과 상황을 읽을 수 있는 1600×900 회색상자
- 고정 시드 RNG, 60Hz 커스텀 시뮬레이션, 인과 EventLog
- 즉시 재시작, 결과 요약, NaN/INF 검사
- 헤드리스 smoke test
- 첫 재미 검증 목표: **3개 병사·3개 보급 역할, 3라인, 생산·배송·요청·라인 이탈·5페이즈 Hive 공략이 작동하는 회색상자**
- 상용 핵심 루프 강화: 역할별 이동 관성, 사격 반동·명중 콤보, 팀 사기, 보급 과열, 클러치·오폭 판정을 가진 2인 수동 폭파

회색상자는 최종 콘텐츠 완성본이 아니라, 최종 구현을 시작하기 전에 핵심 재미를 직접 느끼고 수치 방향을 잡는 **실행 가능한 M0**다. 최종 재현 범위는 `docs/`와 **4,653개 체크리스트**가 정답이다.
