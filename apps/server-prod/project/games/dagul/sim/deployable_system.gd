...
func _replace_owner_mine(owner: int) -> void:
...
func update_deployables(dt: float) -> void:
	...
	for mine0 in w.deployables:
		...
		var updated = _update_deployable(mine, dt)
		if updated != null:
			kept.append(updated)
	w.deployables = kept

func _update_deployable(mine: Dictionary, dt: float) -> Variant:
	if StringName(mine.get("type", &"mine")) == &"wall":
		return _update_wall(mine, dt)
...
func _update_wall(mine: Dictionary, dt: float) -> Variant:
