...
func try_mobility(slot: int, direction: Vector2) -> void:
	...
	_mobility_break_combo(h, escaped_combo, old_pos, dir)
	_mobility_effect_for(slot, h, equipment_id, old_pos, distance, dir)
	h = w.heroes[slot]
	w.heroes[slot] = h
	w.event_log.emit(w.tick, &"mobility_used", slot, -1, {"equipment":equipment_id, "name":equipment["mobility_name"]})

func _mobility_break_combo(h: Dictionary, escaped_combo: bool, old_pos: Vector2, dir: Vector2) -> void:
...
func _mobility_effect_for(slot: int, h: Dictionary, equipment_id: String, old_pos: Vector2, distance: float, dir: Vector2) -> void:
	match equipment_id:
		"scatter":
		...
		"mortar":
			h = _mobility_blast_hop(slot, h, old_pos, distance, dir)
		"leech":
...
func _mobility_blast_hop(slot: int, h: Dictionary, old_pos: Vector2, distance: float, dir: Vector2) -> Dictionary:
...
func try_use_active_item(slot: int) -> void:
	...
	if kind == "":
		return
	_use_held_item(slot, h, kind)

func _use_held_item(slot: int, h: Dictionary, kind: String) -> void:
	match kind:
		"medkit":
			_use_medkit_item(slot, h)
		"spring":
			_use_spring_item(slot, h)
		"slide":
			h["held_item"] = ""
...
func _use_medkit_item(slot: int, h: Dictionary) -> void:
...
func _use_spring_item(slot: int, h: Dictionary) -> void:
	h["held_item"] = ""
