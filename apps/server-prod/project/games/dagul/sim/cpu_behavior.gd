...
func _update_cpu(slot: int, dt: float) -> void:
	...
	if float(h.get("slide_time", 0.0)) > 0.0:
		h = _cpu_slide(slot, h, dt)
	h = _cpu_think(slot, h, dt)
	w.heroes[slot] = h
	if float(h.get("ultimate_charge", 0.0)) >= w.ULTIMATE_MAX - 0.5 and not w.roul.hero_has_timed(h, "turtle"):
		if cpu_try_ultimate(slot):
			h = w.heroes[slot]
	_cpu_attack_targets(slot, h)

func _cpu_downed(slot: int, h: Dictionary) -> void:
	h["vel"] = Vector2.ZERO
	h["action"] = &"DOWN"
	w.heroes[slot] = h

func _cpu_stunned(slot: int, h: Dictionary) -> void:
	h["vel"] = Vector2.ZERO
...
func _cpu_combo_capture(slot: int, h: Dictionary) -> Dictionary:
	h["vel"] = Vector2.ZERO
	w.heroes[slot] = h
...
func _cpu_slide(slot: int, h: Dictionary, dt: float) -> Dictionary:
...
func _cpu_think(slot: int, h: Dictionary, dt: float) -> Dictionary:
	h["think"] = float(h["think"]) - dt
	if float(h["think"]) <= 0.0:
		h["think"] = 0.16 + w.rng.rangef(0.0, 0.10)
		_cpu_retarget(slot, h)
		_cpu_pick_move(slot, h)
		var hazard_escape = hazard_escape_vector(slot)
		if hazard_escape.length_squared() > 0.1:
			_cpu_dodge(slot, h, hazard_escape)
	return h

func _cpu_retarget(slot: int, h: Dictionary) -> void:
	...
	if float(h["target_hold"]) <= 0.0 or old_target < 0 or not target_valid(old_target):
		...
		if old_target >= 0 and old_target != target:
			_emit_target_change(slot, target, old_target)

func _emit_target_change(slot: int, target: int, old_target: int) -> void:
...
func _cpu_pick_move(slot: int, h: Dictionary) -> void:
	...
	if heal_index >= 0:
		_cpu_seek_heal(slot, h, heal_index, tslot)
		...
		_cpu_seek_orb(slot, h, orb_index)
		...
		_cpu_seek_crate(slot, h, crate_index)
	elif tslot >= 0:
		_cpu_engage(slot, h, tslot)

func _cpu_seek_heal(slot: int, h: Dictionary, heal_index: int, tslot: int) -> void:
...
func _cpu_seek_orb(slot: int, h: Dictionary, orb_index: int) -> void:
...
func _cpu_seek_tower(slot: int, h: Dictionary) -> void:
...
func _cpu_seek_crate(slot: int, h: Dictionary, crate_index: int) -> void:
...
func _cpu_engage(slot: int, h: Dictionary, tslot: int) -> void:
...
func _cpu_dodge(slot: int, h: Dictionary, hazard_escape: Vector2) -> void:
...
func _cpu_attack_targets(slot: int, h: Dictionary) -> void:
	h = _cpu_attack_current_target(slot, h)
...
func _cpu_attack_crate(slot: int, h: Dictionary) -> Dictionary:
...
func hazard_escape_vector(slot: int) -> Vector2:
	...
	escape += _escape_from_zones(slot, hero_pos)
	escape += _escape_from_projectiles(slot, hero_pos)
	escape += _escape_from_deployables(slot, hero_pos)
	escape += _escape_from_zone_edge(slot, hero_pos)
	return escape.normalized() if escape.length_squared() > 0.1 else Vector2.ZERO

func _escape_from_zones(slot: int, hero_pos: Vector2) -> Vector2:
	var escape := Vector2.ZERO
	for zone in w.zones:
...
func _escape_from_projectiles(slot: int, hero_pos: Vector2) -> Vector2:
	var escape := Vector2.ZERO
...
func _escape_from_deployables(slot: int, hero_pos: Vector2) -> Vector2:
	var escape := Vector2.ZERO
	for mine in w.deployables:
		if int(mine["owner"]) == slot:
			continue
		if StringName(mine.get("type", &"mine")) == &"wall":
			escape += _escape_wall_dodge(slot, hero_pos, mine)
			continue
		if float(mine.get("arm_time", 0.0)) > 0.0:
			continue
		escape += _escape_mine_dodge(slot, hero_pos, mine)
	return escape

func _escape_wall_dodge(slot: int, hero_pos: Vector2, mine: Dictionary) -> Vector2:
	...
	if forward_distance >= -w.HERO_RADIUS and forward_distance <= minf(remaining_sweep, 480.0) + w.HERO_RADIUS and side_distance <= float(mine["half_length"]) + w.HERO_RADIUS + 45.0:
		...
		return dodge_side * (1.15 if float(mine.get("arm_time", 0.0)) > 0.0 else 1.75)
	return Vector2.ZERO

func _escape_mine_dodge(slot: int, hero_pos: Vector2, mine: Dictionary) -> Vector2:
	...
	return away * (1.0 - distance / danger_radius + urgency)

func _escape_from_zone_edge(slot: int, hero_pos: Vector2) -> Vector2:
...
func cpu_try_ultimate(slot: int) -> bool:
	...
	var want := _ultimate_wanted(animal, dist, hp_ratio)
...
func _ultimate_wanted(animal: int, dist: float, hp_ratio: float) -> bool:
	var want := false
	match animal:
		0:
			want = dist < 520.0
...
func _pull_near_count(slot: int, h: Dictionary) -> int:
	var near := 0
...
func cpu_consider_held_item(slot: int) -> void:
	...
	match kind:
		...
		"pull":
			should_use = _pull_near_count(slot, h) > 0 and w.rng.chance(0.12)
		"spring":
