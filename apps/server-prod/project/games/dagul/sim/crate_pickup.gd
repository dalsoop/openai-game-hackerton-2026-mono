...
func place_crate_ring(count: int, radius_x: float, radius_y: float, rot: float, ring_id: int, red_first: bool) -> void:
	...
	for scale_try in scales:
		if not _ring_blocked(count, radius_x, radius_y, rot, scale_try):
...
func _ring_blocked(count: int, radius_x: float, radius_y: float, rot: float, scale_try: float) -> bool:
	for n in range(count):
		var ang := -PI * 0.5 + rot + TAU * float(n) / float(count)
		var dir: Vector2 = Vector2.RIGHT.rotated(ang)
...
func update_crate_orbs(dt: float) -> void:
	...
	for orb0 in w.crate_orbs:
		...
		if float(orb["arm"]) <= 0.0 and _steer_orb_to_target(orb, dt):
			continue
		kept.append(orb)
	w.crate_orbs = kept

func _steer_orb_to_target(orb: Dictionary, dt: float) -> bool:
...
func _steer_pickup(pickup: Dictionary, pickup_index: int, dt: float) -> Dictionary:
	...
	if magnet_slot >= 0:
		...
		if Vector2(pickup["pos"]).distance_to(target_pos) <= w.HERO_RADIUS + w.HEALTH_PICKUP_RADIUS:
			_collect_pickup(pickup, magnet_slot, target_pos, pickup_index)
	else:
		pickup["pos"] = Vector2(pickup["pos"]).move_toward(Vector2(pickup["home"]), w.HEALTH_PICKUP_RETURN_SPEED * dt)
	return pickup

func _collect_pickup(pickup: Dictionary, magnet_slot: int, target_pos: Vector2, pickup_index: int) -> Dictionary:
	var h: Dictionary = w.heroes[magnet_slot]
	var carried := int(h.get("medkits", 0))
	if w.mode == w.ITEM_POOL_MODE:
		return w.act_item.collect_item_pickup(magnet_slot, pickup)
		...
		pickup["magnet_slot"] = -1
		pickup["pos"] = Vector2(pickup["home"])
		w.proj.add_effect(&"heal_pickup", target_pos, 64.0, 0.38, Color("#6ef3a5"), "메드킷 +1")
