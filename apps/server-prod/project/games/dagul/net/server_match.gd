...
const NetworkHostScript = preload("res://games/dagul/net/network_host.gd")
const ReplayRecorderScript = preload("res://games/dagul/net/replay_recorder.gd")
const LagCompensatorScript = preload("res://games/dagul/net/lag_compensator.gd")
...
func _check_disconnection_timeouts() -> void:
	...
	for slot in _disconnected_at.keys():
		if now - _disconnected_at[slot] > 30000:
			_disconnected_at.erase(slot)
			_erase_slot_token(slot)

func _erase_slot_token(slot: int) -> void:
