...
server-*-dev2/      그 사람의 개발환경 2. 호스트·허브는 이미 있음
server-*-dev3/      그 사람의 개발환경 3. 호스트·허브는 이미 있음
server-prod/        제출·운영 슬롯. 웹은 아직 끔
...
사람마다 `dev1`/`dev2`/`dev3`가 있다. 폴더마다 호스트와 허브가 따로다. `dev2`/`dev3`는 웹만 아직 꺼 둔 상태다. 거기에 올릴 때는 그 폴더에서 작업하고 `web.enabled`를 켠다. 작업은 `apps/` 안에서만 한다.
