...
func _snap_players() -> Array:
	...
	for h in world.heroes:
		arr.append({
			...
			"hp": float(h["hp"]), "maxHp": float(h.get("max_hp", 100.0)),
			"alive": bool(h["alive"]),
			"weapon": str(h.get("equipment", {}).get("name", "")),
			"mag": int(h.get("mag", 0)),
			"magMax": int(h.get("equipment", {}).get("mag_size", 0)),
			"reloadLeft": float(h.get("reload_left", 0.0)),
			"ult": float(h.get("ultimate_charge", 0.0)),
			"animal": int(h.get("animal", int(h["slot"]))),
