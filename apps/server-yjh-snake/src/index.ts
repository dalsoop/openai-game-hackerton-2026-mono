...
createServer((req, res) => {
  if (req.url === "/health") { res.writeHead(200, {"Content-Type":"application/json"}); res.end(JSON.stringify({ok:true,slot:"server-yjh-snake"})); return; }
  let file = join(PUBLIC, req.url === "/" ? "index.html" : req.url!);
  if (!existsSync(file)) { res.writeHead(404); res.end("Not Found"); return; }
  const mime = MIME[extname(file)] || "application/octet-stream";
  res.writeHead(200, {"Content-Type": mime});
  res.end(readFileSync(file));
}).listen(PORT, () => console.log(`Snake Arena on :${PORT}`));
