...
3. P1: **WASD 이동 · Space 준비/대시 · E 채취·줍기·예치**
4. P2: **방향키 이동 · Enter 준비/대시 · Shift 채취·줍기·예치**
...
- 로컬 인간 **2명** + CPU **4명** (두 사람 준비 전에는 경기 정지)
- 아트 없이도 핵심 판정과 상황을 읽을 수 있는 1600×900 회색상자
- 고정 시드 RNG, 60Hz 커스텀 시뮬레이션, 인과 EventLog
- 즉시 재시작, 결과 요약, NaN/INF 검사
- 헤드리스 smoke test
- 첫 재미 검증 목표: **6인 경쟁, 중앙 광물, 소유권 경합, 드롭·줍기·은행 예치·세 경로·CPU 가로막기가 작동하는 회색상자**
- 상용 핵심 루프 강화: 가속·제동, 대시 입력 버퍼와 빗나감 경직, 반복 괴롭힘 파울, 강탈 복수 대시, 예치 경합, 연속 강탈 핫 코인, 26초 골든 하이스트

회색상자는 최종 콘텐츠 완성본이 아니라, 최종 구현을 시작하기 전에 핵심 재미를 직접 느끼고 수치 방향을 잡는 **실행 가능한 M0**다. 최종 재현 범위는 `docs/`와 **3,193개 체크리스트**가 정답이다.
