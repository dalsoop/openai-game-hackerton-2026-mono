...
static func vis_tile_rect(vis: Rect2, cell: float) -> Rect2i:
	if cell <= 0.0:
		return Rect2i()
	var x0 := tile_coord(vis.position.x, cell)
	var y0 := tile_coord(vis.position.y, cell)
	var x1 := int(ceili(vis.end.x / cell))
	var y1 := int(ceili(vis.end.y / cell))
