...
func draw_head_roulette(body_pos: Vector2, hero: Dictionary) -> void:
	...
	var current := 0
	if spin_id != "":
		current = _roulette_spin_index(spin_id, slice_ids)
	...
	_draw_roulette_slices(wheel_pos, radius, slice, rot, count, cols)
	...
	_draw_roulette_icons(wheel_pos, radius, slice, rot, count, slice_ids, phase, current)
...
func _roulette_spin_index(spin_id: String, slice_ids: Array) -> int:
	var current := 0
	for i in range(slice_ids.size()):
		if str(slice_ids[i]) == spin_id:
			current = i
			break
	return current

func _draw_roulette_slices(wheel_pos: Vector2, radius: float, slice: float, rot: float, count: int, cols: Array) -> void:
...
func _draw_roulette_icons(wheel_pos: Vector2, radius: float, slice: float, rot: float, count: int, slice_ids: Array, phase: String, current: int) -> void:
	for i in range(count):
...
func draw_reload_bubble(body_pos: Vector2, hero: Dictionary) -> void:
	...
	if r.reload_bubble_atlas != null:
		_draw_reload_bubble_atlas(body_pos, hero, mode)
		return
	_draw_reload_bubble_fallback(body_pos, mode)

func _draw_reload_bubble_atlas(body_pos: Vector2, hero: Dictionary, mode: String) -> void:
	var frame := 8
	if mode == "RELOADING":
...
func _draw_reload_bubble_fallback(body_pos: Vector2, mode: String) -> void:
	var bw := 36.0
	var bh := 28.0
...
func _draw_reload_spinner(c: Vector2, p: int) -> void:
	for i in range(8):
...
func draw_finish_cine() -> void:
	...
	if r.finish_bg_tex == null:
		r.finish_bg_tex = r._load_tex("res://games/dagul/assets/fx/finish-bg.png")
