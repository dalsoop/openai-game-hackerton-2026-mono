...
static func build(hud_node: Node, on_exit: Callable, on_rematch: Callable) -> Dictionary:
	...
	var exit_btn := make_button(HudStrings.t("btn_exit"), UiTheme.BTN_DARK)
	...
	var rematch_btn := make_button(HudStrings.t("btn_rematch"), UiTheme.BLUE)
...
static func sync(exit_btn: Button, rematch_btn: Button, touch, phase: StringName, world, net_active: bool) -> void:
	...
	exit_btn.text = HudStrings.t("btn_to_lobby") if finished else HudStrings.t("btn_exit")
