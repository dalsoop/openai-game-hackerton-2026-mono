...
import { handleStart, sendStartBodies, type LobbyBag, type LobbyHandle } from "@/lib/hub/lobby-waiting";
...
describe("매치 시작 — 랜덤 픽 해소가 START 보다 먼저 끝난다", () => {
  it("host 가 랜덤을 두고 시작해도 players.characterId 는 실제 동물로 해소된다", () => {
    ...
    const started = handleStart(room, bag, { sessionId: "host" } as unknown as Client);
    expect(started).toBe(true);
    expect(bag.authority).toBeNull(); // handleStart 는 아직 해소하지 않는다
  ...
  it("START 페이로드의 seats[].characterId 는 unknown 을 보내지 않는다", () => {
    ...
    handleStart(room, bag, { sessionId: "host" } as unknown as Client);
    bootAuthority(room, bag);
