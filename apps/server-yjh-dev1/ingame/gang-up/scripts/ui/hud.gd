...
func _draw_dash_slot(rect: Rect2, me: Dictionary, _equipment: Dictionary) -> void:
    ...
    if ready:
        _text(rect.position + Vector2(48.0, 40.0), tr("HUD_DASH_READY"), 15, Color.WHITE)
    else:
...
func _draw_controls_overlay() -> void:
    ...
    var lines := [tr("HUD_CONTROLS_MOVE"), tr("HUD_CONTROLS_AIM"), tr("HUD_CONTROLS_FIRE"), tr("HUD_CONTROLS_SKILL"), tr("HUD_CONTROLS_DASH")]
    for i in lines.size():
...
func _update_kill_feed() -> void:
    ...
    for ev in world.event_log.events:
        ...
        var label := tr("HUD_KILL_BY") % [killer_name, victim_name] if killer_name != "" else tr("HUD_KILL_ELIMINATED") % victim_name
        if t == &"player_eliminated":
            label += " " + tr("HUD_KILL_FINISHED")
