...
static func build(close_callback: Callable) -> Control:
	...
	col.add_child(UiTheme.lbl(tr("HOW_SUBTITLE"), 14, UiTheme.MUTED))
	col.add_child(UiTheme.lbl(tr("HOW_TITLE_FULL"), 28, UiTheme.INK))
	for line in [
		tr("HOW_KB_LINE"),
		tr("HOW_FIRE_LINE"),
		tr("HOW_EXTRA_LINE"),
	...
	var back := UiTheme.btn(tr("HOW_BACK"), UiTheme.BTN_DARK, Vector2(140, 48))
