...
const _STATUS_COLORS := {
	"lobby": UiTheme.GREEN,
	"offline": UiTheme.ERROR,
	"closed": UiTheme.ERROR,
	"reconnecting": UiTheme.WARN,
	"connecting": UiTheme.WARN,
}
...
func _ready() -> void:
	...
	_build()
	_sync_settings_ui()
	var default_name := tr("FLOW_DEFAULT_NAME") % (randi() % 90 + 10)
...
func _build_intro() -> Control:
	...
	var subtitle := UiTheme.lbl(tr("GAME_SUBTITLE"), 18, UiTheme.INTRO_SUB, HORIZONTAL_ALIGNMENT_CENTER)
	...
	_intro_name_edit.placeholder_text = tr("INTRO_NAME_PLACEHOLDER")
	...
	var hint := UiTheme.lbl(tr("INTRO_HINT"), 12, UiTheme.INTRO_SUB, HORIZONTAL_ALIGNMENT_CENTER)
...
func _on_create_pressed() -> void:
	if hub == null:
		_lobby_error.text = tr("MSG_CONNECT_FAIL")
		return
	_push_identity()
	if hub.is_open():
	...
	_lobby_error.text = tr("MSG_CONNECTING")
	if hub.status == "offline" or hub.status == "closed":
		hub.reconnect_now()
	else:
		hub.ensure_connected()
...
func _rebuild_room_list() -> void:
	...
	if hub == null or not hub.is_open():
		_room_list.add_child(UiTheme.lbl(tr("MSG_CONNECTING"), 15, UiTheme.MUTED))
		return
...
func _on_hub_status(next: String) -> void:
	...
	if _retry_button != null:
		_retry_button.visible = next == "offline" or next == "closed"
	_try_pending_create(next)
	if page == &"wait":
		_fill_wait()

func _try_pending_create(status: String) -> void:
	if page == &"lobby" and status == "lobby" and hub != null:
		hub.request_rooms()
		...
		hub.create_room()
		return
	if status != "offline" or not _pending_create:
		return
	_pending_create = false
	if _lobby_error != null:
		_lobby_error.text = tr("MSG_CONNECT_FAILED")
...
func _update_wait_mode_label(_mode_id: String) -> void:
	if _wait_mode_label == null:
		return
	if hub != null and hub.in_room:
		_wait_mode_label.text = "%s  |  %s" % [str(hub.room.get("title", tr("LOBBY_ROOM_DEFAULT"))), tr("MSG_WAIT_MODE")]
	else:
		_wait_mode_label.text = tr("MSG_OFFLINE_PRACTICE")
...
func _fill_slot(i: int, online: bool, me_host: bool) -> void:
	...
	if online and i < hub.players.size():
		...
		nick.text = tr("FLOW_PEER_ME") % _display_name()
...
func _fill_slot_online(i: int, nick: Label, ready: Label) -> void:
	...
	if i == hub.you:
		peer_name = tr("FLOW_PEER_ME") % peer_name
	nick.text = peer_name
...
func _update_start_button() -> void:
	...
	if show_start:
		_start_button.text = tr("FLOW_RETURN_GAME") if (online and hub.match_running) else tr("FLOW_START_GAME")
...
func _show_rules_card() -> void:
	...
	vbox.add_child(UiTheme.lbl(tr("RULES_TITLE"), 28, UiTheme.INK, HORIZONTAL_ALIGNMENT_CENTER))
	var rules := [
		tr("RULES_LINE1"),
		tr("RULES_LINE2"),
		tr("RULES_LINE3"),
	...
	card.add_child(vbox)
	var close_btn := UiTheme.btn(tr("RULES_CLOSE"), UiTheme.BLUE, Vector2(120, 44))
