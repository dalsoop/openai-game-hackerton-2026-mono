...
func draw_flee_mark(body_pos: Vector2, hero: Dictionary) -> void:
	...
	r.draw_string(GameFont.get_font(), mark + Vector2(-30.0, 28.0), tr("RENDER_FLEEING"), HORIZONTAL_ALIGNMENT_CENTER, 60.0, 12, Color("#ffe066"))
...
func draw_snake_skins() -> void:
	for skin in world.snake_skins:
		...
		r.draw_string(GameFont.get_font(), pos + Vector2(-36.0, -76.0), tr("RENDER_SHELL"), HORIZONTAL_ALIGNMENT_CENTER, 72.0, 13, Color("#d8f5c4"))
...
func draw_heroes() -> void:
	for hero in world.heroes:
		...
		if int(hero.get("kill_streak", 0)) >= 2:
			r.draw_string(GameFont.get_font(), body_pos + Vector2(-40.0, -62.0), tr("RENDER_KILL_STREAK") % int(hero["kill_streak"]), HORIZONTAL_ALIGNMENT_CENTER, 80.0, 11, Color("#ffd166"))
