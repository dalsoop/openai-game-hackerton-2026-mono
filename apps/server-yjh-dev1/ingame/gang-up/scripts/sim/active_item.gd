...
func try_use_medkit(slot: int) -> void:
	...
	w.heroes[slot] = h
	w.dmg.heal_hero(slot, heal_amount)
	w.proj.add_effect(&"heal_pickup", Vector2(h["pos"]), 72.0, 0.45, Color("#6ef3a5"), tr("MEDKIT_USE") % roundi(heal_amount))
...
func try_gun_loot(owner: int, attacker: Dictionary) -> Dictionary:
	...
	w._announce(tr("ITEM_ACQUIRED") % [owner + 1, str(attacker["equipment"]["name"])], 90)
	return attacker
