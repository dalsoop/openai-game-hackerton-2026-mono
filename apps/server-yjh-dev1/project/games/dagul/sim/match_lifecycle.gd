...
func update_timers(dt: float) -> void:
	...
	for i in range(w.heroes.size()):
		...
		_hero_timer_cools(h, dt)
		_hero_timer_states(h, dt)
		_hero_timer_reload(h, dt)
		w.heroes[i] = h
...
func _hero_timer_states(h: Dictionary, dt: float) -> void:
...
func _hero_timer_reload(h: Dictionary, dt: float) -> void:
...
func tick_downs(dt: float) -> void:
	for slot in range(w.heroes.size()):
		_tick_downs_slot(slot, dt)

func _tick_downs_slot(slot: int, dt: float) -> void:
	var h: Dictionary = w.heroes[slot]
	if not bool(h.get("downed", false)):
		return
	if not bool(h.get("alive", false)):
...
func down_hero(owner: int, target: int) -> void:
	...
	if death_velocity.length() < 450.0:
		death_velocity = _death_direction(owner, target, h) * 1550.0
	else:
	...
	_down_hero_respawn(h, target, used, final_out)
	w.heroes[target] = h
	...
	_announce_down_result(owner, target, h)
	w.impact_ticks = maxi(w.impact_ticks, 32)

func _death_direction(owner: int, target: int, h: Dictionary) -> Vector2:
	...
	return death_direction

func _down_hero_reset(h: Dictionary) -> void:
	h["alive"] = false
	h["hp"] = 0.0
	h["downed"] = false
	h["down_left"] = 0.0
...
func _down_hero_respawn(h: Dictionary, target: int, used: int, final_out: bool) -> void:
	if final_out:
		h["eliminated"] = true
		h["respawn"] = 0.0
		h["respawn_left"] = 0.0
...
func _announce_down_result(owner: int, target: int, h: Dictionary) -> void:
...
func _reward_attacker(owner: int, target: int, defeated_streak: int, bounty_victim: bool, life_hits: Dictionary, victim: Dictionary) -> Dictionary:
	...
	if bool(attacker["alive"]):
		streak_after = _reward_alive_streak(attacker)
	if defeated_streak >= 3:
		...
		show_streak_callout("연속 처치 종료!", "P%d %s님이 P%d %s님의 %d연속 처치를 끝냈습니다." % [owner + 1, attacker_name, target + 1, defeated_name, defeated_streak], true)
		...
		show_streak_callout(streak_title(streak_after), "P%d %s님이 %d연속 처치 중입니다." % [owner + 1, streak_attacker_name, streak_after], false)
...
func _reward_alive_streak(attacker: Dictionary) -> int:
	var streak_after = int(attacker.get("kill_streak", 0)) + 1
