...
func draw_effects_main() -> void:
	...
	for effect in world.effects:
		...
		var mobility_texture =r._mobility_fx_texture(effect_kind, effect_label)
		if effect_label == "SIGHTLINE STEP" and effect_kind == &"beam_step" and r.rooster_beam_step_atlas != null:
		...
		if str(effect["label"]) != "" and effect_kind in [&"heal_pickup", &"respawn"]:
			r.draw_string(GameFont.get_font(), effect_pos + Vector2(-100.0, -effect_radius - 10.0), str(effect["label"]), HORIZONTAL_ALIGNMENT_CENTER, 200.0, 16, Color(effect_color, ratio))
