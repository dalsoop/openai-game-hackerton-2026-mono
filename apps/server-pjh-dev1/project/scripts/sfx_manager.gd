...
func process_events(world, local_slot: int, last_event_id: int) -> Dictionary:
	...
	for event in world.event_log.events:
		...
		if event_type == &"gun_fire":
			...
			if actor == local_slot:
				Audio.play_sfx(fire_id, -1.0, 0.03)
				print("[gangup] sfx %s eq=%s" % [fire_id, eq])
			else:
				Audio.play_sfx_at(fire_id, _hero_pos(world, actor), -2.0, 0.03)
		elif event_type == &"reload_started":
			var req := str(event["data"].get("equipment", ""))
			Audio.play_sfx(SfxCatalog.reload_id_for(req), -4.0, 0.02) if actor == local_slot else Audio.play_sfx_at(SfxCatalog.reload_id_for(req), _hero_pos(world, actor), -5.0, 0.02)
			if actor == local_slot:
		...
		if event_type == &"hero_hit" and involves:
			...
			Audio.play_sfx("roulette", -4.0, 0.0)
