...
func _draw_crosshair(me: Dictionary) -> void:
    ...
    for thick in [3.4, 1.6]:
        ...
        draw_circle(c, (2.6 if thick > 2.0 else 1.7) * s, col if thick > 2.0 else accent)
        draw_line(c + Vector2(0, -gap - arm), c + Vector2(0, -gap), col, thick * s)
        draw_line(c + Vector2(0, gap), c + Vector2(0, gap + arm), col, thick * s)
        draw_line(c + Vector2(-gap - arm, 0), c + Vector2(-gap, 0), col, thick * s)
        draw_line(c + Vector2(gap, 0), c + Vector2(gap + arm, 0), col, thick * s)
