...
func _physics_process(_delta: float) -> void:
    ...
    var command := {
        ...
        "p1":{"move":p1_move, "build":_edge(KEY_SPACE), "bomb":_edge(KEY_Q), "web":_edge(KEY_E), "interact":Input.is_key_pressed(KEY_G)},
        "p2":{"move":p2_move, "build":_edge(KEY_ENTER), "bomb":_edge(KEY_SLASH), "web":_edge(KEY_PERIOD), "interact":Input.is_key_pressed(KEY_SHIFT)}
    }
    world.step_tick(command, 1.0 / 60.0)
    var shake := Vector2.ZERO
    if world.impact_ticks > 0:
        shake = Vector2(sin(float(world.tick) * 2.1), cos(float(world.tick) * 3.4)) * (2.0 + world.impact_ticks * 0.35)
    camera.position = camera.position.lerp(world.camera_target(), 0.16) + shake
