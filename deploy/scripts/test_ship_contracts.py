...
class PlatformGodotPipeline(unittest.TestCase):
    def test_next_slots_export_on_ship(self) -> None:
        ...
        self.assertIn("dagul-game-dashboard.json", dash_cm)
        dash = (root / "deploy" / "chart" / "files" / "dagul-game-dashboard.json").read_text()
        self.assertIn('"uid": "dagul-game"', dash)
        self.assertIn("서버 정원", dash)
        self.assertIn("Asia/Seoul", dash)
        self.assertIn("dagul_ccu", dash)
        self.assertIn("grafana.50.internal.kr", readme)
        self.assertIn("ErrorBanner", (APPS / "dagul-prod" / "web" / "app" / "[locale]" / "page.tsx").read_text())
