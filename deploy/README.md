...
개발자는 허브 코드를 `apps/`에 두고 푸시한다. 이미지와 Helm은 PVE의 `hackertone` 러너가 하고, Cloudflare 퍼지는 GitHub 호스트가 한다. Godot wasm/pck는 CI가 올리지 않는다.
...
1. PVE에 GitHub 러너 `pve-hackertone` (라벨 `hackertone`). `docker`와 `10.0.50.100` SSH가 되면 된다.
